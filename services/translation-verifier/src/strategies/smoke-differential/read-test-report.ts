...
export async function readReport<T>(dir: string, validate?: (raw: unknown) => asserts raw is T): Promise<T> {
  ...
  try {
    if (statSync(reportPath).size > MAX_REPORT_BYTES) {
      throw new SmokeVerificationError("report_schema_invalid", `报告文件 report.json 超过大小上限(${MAX_REPORT_BYTES} 字节)`);
    }
    ...
    throw new SmokeVerificationError(missing ? "report_missing" : "report_schema_invalid", `无法读取报告文件 ${reportPath}: ${message}`, { cause: err });
  }
  let raw: unknown;
  try {
    raw = JSON.parse(text);
  } catch (err) {
    throw new SmokeVerificationError("report_invalid_json", `报告文件不是合法 JSON ${reportPath}: ${(err as Error).message}`, { cause: err });
  }
  if (validate) {
...
function applyValidate<T>(raw: unknown, validate: (value: unknown) => asserts value is T): void {
  try {
    ...
    throw new SmokeVerificationError("report_schema_invalid", errorSummary(error), { cause: error });
  }
}
