...
const job: SmokeTaskInput = {
  ...
  source: {
    ...
    files: [
      {
        relativePath: "identity.ts",
        content: "export const identity = (x: string) => x;\n",
      },
    ],
  },
  target: {
...
describe("prepared smoke task", () => {
  it("constructs prompts and session settings without copying, writing or re-baselining projects", () => {
    ...
    try {
      ...
      expect(task.llm.allowedTools).toEqual([
        `Bash(npx tsx ${join(packageRoot, "src/strategies/smoke-differential/controlled-test-command.ts")} *)`,
      ]);
  ...
  it("keeps caller-owned prepared layouts unchanged and preserves cancellation identity", () => {
    ...
    try {
      ...
      const options = {
        workspaceDir: prepared.agentDir,
        executionRoot: prepared.executionRoot,
        baselinePath: prepared.baselinePath,
        commandEvidencePath: prepared.evidencePath,
        runnerRoots: prepared.runnerRoots,
      };
      const callerJob = {
        ...job,
        source: { ...job.source, root: prepared.projectRoots[0] },
        target: { ...job.target, root: prepared.projectRoots[1] },
      };
      const before = snapshot(workspace.dir);
      ...
      expect(() =>
        prepareAgentTask(callerJob, options, layout, AbortSignal.abort(error)),
      ).toThrow(error);
      expect(snapshot(workspace.dir)).toEqual(before);
    } finally {
      rmSync(root, { recursive: true, force: true });
