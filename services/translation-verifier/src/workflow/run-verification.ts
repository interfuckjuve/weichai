...
import { createRunRecorder, stepFailureState, type StepHandle } from "../run-output/record-run.js";
import { validateInput } from "./validate-input.js";
import { createVerificationWorkspace } from "./prepare-strategy-workspace.js";
import { runStrategy } from "./run-strategy.js";
import { VerificationArtifactPersistenceError, type VerificationArtifactStore } from "../run-output/verification-artifact-store.js";
...
import type { VerificationInput, VerificationReceipt, VerificationResult } from "../schemas/verification-types.js";
...
export async function runVerification(
  ...
  return withVerificationTimingRecorder(recorder, async () => {
    let store: Pick<VerificationArtifactStore, "writtenArtifacts" | "writeFrameworkResult" | "cleanup"> | undefined;
    ...
    try {
      ...
      saveHandle ??= recorder.startStep("save-report", { scope: "framework" });
      const cleanup = recorder.startStep("workspace-cleanup", { scope: "framework", parentId: saveHandle?.id });
      try {
        ...
        recorder.endStep(saveHandle, saveFailure === undefined ? "completed" : stepFailureState(saveFailure), saveFailure);
        recorder.finish();
      }
    }
  });
