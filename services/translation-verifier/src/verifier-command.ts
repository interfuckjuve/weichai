...
function containedRealPath(root: string, candidate: string, label: string): string {
  const rootLexical = resolve(root);
  const candidateLexical = resolve(candidate);
  if (candidateLexical !== rootLexical && !candidateLexical.startsWith(rootLexical + sep)) {
  ...
  if (existsSync(candidateLexical)) {
    ...
    if (physical !== rootReal && !physical.startsWith(rootReal + sep)) {
      throw new Error(`${label} 必须位于工作区内: ${candidate} (实际 ${physical})`);
    }
  }
...
function resolveAllowedCommand(command: string, workspaceRoot: string, cwd: string): string {
 ...
 */
function assertBaselineGated(workspaceRoot: string, baselinePath: string): void {
  try {
    assertWorkspaceBaseline(workspaceRoot, baselinePath);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    if (message.includes("new file outside")) {
      throw new Error(`baseline violation: runner 区外出现新源码(new source): ${message}`);
    }
    throw new Error(`baseline 校验失败: ${message}`);
  }
}

function appendEvidenceLine(evidencePath: string, evidence: CommandEvidence): void {
...
export async function runVerifierCommand(
  ...
  if (side !== "source" && side !== "target") throw new Error(`invalid side: ${String(side)}`);
  if (phase !== "compile" && phase !== "run") throw new Error(`invalid phase: ${String(phase)}`);
  const workspaceRoot = resolve(input.workspaceRoot);
  containedRealPath(workspaceRoot, input.cwd, "cwd");
  assertBaselineGated(workspaceRoot, input.baselinePath);
  const resolvedCommand = resolveAllowedCommand(input.command, workspaceRoot, input.cwd);
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : String(error);
    postBaselineError = /^baseline/.test(message) ? message : `baseline 校验失败: ${message}`;
  }
...
type CliParse = { ok: true; invocation: CliInvocation } | { ok: false; message: string };
...
function parseCliInvocation(argv: string[]): CliParse {
  ...
  if (side === undefined) return { ok: false, message: "缺少 --side source|target" };
  if (side !== "source" && side !== "target") return { ok: false, message: `无效 side: ${side}` };
  if (phase === undefined) return { ok: false, message: "缺少 --phase compile|run" };
  if (phase !== "compile" && phase !== "run") return { ok: false, message: `无效 phase: ${phase}` };
  if (cwd === undefined || cwd === "") return { ok: false, message: "缺少 --cwd" };
  if (rest.length === 0) return { ok: false, message: "缺少命令(-- 之后为 command args)" };
  return { ok: true, invocation: { side, phase, cwd, command: rest[0], args: rest.slice(1) } };
}
...
export async function runVerifierCommandCli(
  ...
  if (!Number.isFinite(deadlineAt)) {
    process.stderr.write(`verifier-command: VERIFIER_DEADLINE_AT 非法: ${deadlineRaw}\n`);
    return 1;
  }
...
if (isModuleEntryPoint()) {
  try {
    process.exitCode = await runVerifierCommandCli(process.argv.slice(2), process.env);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    process.stderr.write(`verifier-command: ${message}\n`);
    process.exitCode = 1;
