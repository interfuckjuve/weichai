 ...
 *
 * 用例设计(plan)只有自然语言意图描述,不声明 expected 黄金值——一致性判断 =
 * 机械差分 + LLM 语义裁决(由 claude 自主会话在其内部完成)。
 */
/** smoke 差分验证支持的语言。 */
...
export interface WorkspaceBaseline {
  ...
  mutableFiles: ["agent/report.json", "agent/claude-steps.jsonl", "agent/commands.jsonl"];
