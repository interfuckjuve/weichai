...
`prepare-strategy-workspace.ts` stages target snapshots and the translation patch, and stages source files when the selected provider requests them. For two-phase providers, `workspaceRequirements(input, phase)` distinguishes `"prepare-tests"` and `"verify-translation"`. Black-box verification needs only the target project; white-box direct/adapt verification still validates and exposes the source handoff. It does not create smoke baselines, runner layouts or an Agent task sequence. A provider supplies `descriptor` and `create()`, and its strategy exposes either `verify(input, context, signal)` or the explicit `prepareTests`/`verifyTranslation` pair. Two-phase preparation is saved as a Host-issued capsule under the per-attempt artifact budget and indexed by a trusted-preparations receipt under `artifactRoot`; verification rejects capsules not issued by that shared store. Preparation failure evidence is retained unless persistence fails. `context.measureStep()` wraps existing work only; the recorder is not a workflow state machine.

Two-phase calls are explicit: `prepareTests({ request, analysisReport, migrationPlan })` receives no translation and returns a serializable capsule; after translation, `verifyTranslationWithReceipt(input, { strategyId, preparation })` verifies it. The caller owns prepared copies, with an untranslated target for preparation and translated target for verification. `verify()` rejects two-phase providers. Direct strategy calls are trusted programmatic APIs; callers must validate capsule origin. There is no `waitForTarget` readiness API, and local phases do not wire upstream Analyzer/Translator parallel scheduling.
