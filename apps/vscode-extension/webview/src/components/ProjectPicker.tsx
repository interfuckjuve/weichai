...
export function ProjectPicker(props: ProjectPickerProps) {
  ...
  const label = current?.displayName ?? (workspace.projectId ? workspace.name : mode === 'target' ? '选择目标项目' : '选择历史项目');
    ...
    <div className="workspace-picker" ref={container} onKeyDown={(event) => {
      ...
      <div className="workspace-picker-row">
        <button ref={trigger} type="button" className="workspace-picker-trigger" aria-haspopup="menu"
          aria-expanded={open} aria-label={mode === 'target' ? '选择目标项目' : '选择历史项目'}
      ...
      {open ? (
        <div className="workspace-picker-menu" role="menu" aria-label={mode === 'target' ? '目标项目列表' : '历史项目列表'} ref={menu}>
          <div className="workspace-picker-options">
            ...
            {!repositories.length ? <div className="workspace-picker-empty">{mode === 'target' ? '尚未选择目标工程' : '尚未添加历史仓库'}</div> : null}
          </div>
          <div className="workspace-picker-commands">
            ...
            </> : <button type="button" role="menuitem" onClick={() => choose(props.onOpenSettings)}><Settings2 size={14} />管理历史仓库…</button>}
          </div>
        </div>
      ) : null}
    </div>
